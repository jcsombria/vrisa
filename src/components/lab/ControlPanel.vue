...
export default {
  ...
  methods: {
    ...
    synced: (control) => control.value == control.new_value,
    set(control) {
      const values = this.controls.elements.map((name) =>
        parseFloat(this.controllerModel.writables[name].new_value)
      );
      const update = {};
      update[this.controls.name] = values;
      this.labcontrol.set([update]);
      // TODO: actually update
      this.controls.elements.forEach(i => {
        this.controllerModel.writables[i].value =
          this.controllerModel.writables[i].new_value;        
      });
